const listaContrasenas = () => {

    const lista = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
const hallarIncorrecta42 = () => {
    const arrayClavesIncorrectas = listaContrasenas().filter(linea => { //En vez de forEach creo que filter
        const arrayLinea = linea.split(': ');
        const politicaClave = arrayLinea[0];
        const clave = arrayLinea[1];    //

        const rangoYLetra = politicaClave.split(' ');
        const rango = rangoYLetra[0];
        const letra = rangoYLetra[1];   //

        const minimoYMaximo = rango.split('-');
        const minimo = Number(minimoYMaximo[0]);    //
        const maximo = Number(minimoYMaximo[1]);    //

        const arregloLetrasClave = clave.split('');
        const cantidadLetra = arregloLetrasClave.filter(letraClave =>  letraClave === letra).length; //Devuelve la cantidad de apariciones de la letra
        // return (cantidadLetra >= minimo && cantidadLetra <= maximo);
        return (cantidadLetra < minimo || cantidadLetra > maximo);

    });

    const incorrecta42 = arrayClavesIncorrectas[41].split(': ')[1];

    return incorrecta42;
}

console.log(hallarIncorrecta42());